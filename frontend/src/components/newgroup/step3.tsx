    ...
    <div className={ss.teamContent}>
      <div className='flex-col'>
        {teams.map(({ uuid: teamUuid, name, players: teamPlayers }) => (
          <div key={ teamUuid } className={`flex-col ${ss.teamDiv} ${ss.teamDivReal}`}>
            <div className='flex-row'>
              ...
              <input
                className={`${ss.teamnameInput} ${ss.inputBox}`}
              ...
              />
              <button
                className={`${ss.teamMinusButton} ${ss.inputBox}`}
            ...
            <div className={ss.gridTeamPlayernames}>
              {[...teamPlayers, uuidv4()].map((playerUuid, index) => index !== teamPlayers.length ? (
                <div key={ playerUuid } className='playerInTeamDiv'>
                  <input
                ...
                <div key={ playerUuid } className={ss.playerInTeamDiv}>
                  <input
                    className={`${ss.playernameInTeam} ${ss.inputBox} ${((teamPlayers.length === 0) && (elem.props.children === emptyTeamError) ? ss.errorInput : '')}`}
                    type="text"
                    value=''
                    list="namelist"
      ...
      <div className={`flex-row ${ss.teamListSample}`}>
        <div className={`flex-col ${ss.teamDiv}`}>
          <div className='flex-row'>
            ...
            <button
              className={`${ss.addButtonInTeam} ${ss.addTeamButton} ${ss.inputBox}`}
