...
function New() {
  ...
  return (
    <div className='flex-col step-form' style={{'--playername-columns': playernameColumns} as CSSProperties}>
      ...
      {((currentStep === 1) && !doTeams) || (currentStep === 2 && doTeams) ? <Step2 groupInfo={groupInfo} players={players} setPlayers={setPlayers} teams={teams} setTeams={setTeams} doTeams={doTeams} playernameColumns={playernameColumns} setPlayernameColumns={setPlayernameColumns}/> : <></>}
