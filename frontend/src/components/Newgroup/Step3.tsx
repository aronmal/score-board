...
import { addPlayerToTeam, addTeam, changePlayername, changeTeamname, checkForPlayernameDuplicates, checkForTeamnameDuplicates, isLastPlayerInTeam, playernameOfUuid, removePlayerFromTeam, removeTeam, replacePlayerWithUuidTo, validate } from './Helpers';

function Step3({ elem, players, setPlayers, teams, setTeams, playernameColumns, setPlayernameColumns }: { elem: JSX.Element, players: playerType[], setPlayers: Dispatch<SetStateAction<playerType[]>>, teams: teamType[], setTeams: Dispatch<SetStateAction<teamType[]>>, playernameColumns: number, setPlayernameColumns: Dispatch<SetStateAction<number>> }) {
          ...
          <div key={ team.uuid } className='flex-col team-div team-div-real'>
            <div className='flex-row'>
              ...
              <input
                ...
                onChange={e => changeTeamname(teams, setTeams, validate(e.target.value), team.uuid)}
              />
              <button
              ...
              {team.players.map(playerUuid => (
                <div key={ playerUuid } className='player-in-team-div'>
                  <input
                    ...
                    onChange={e => {
                      if ((isLastPlayerInTeam(team, playerUuid)) && !playernameOfUuid(players, playerUuid) && validate(e.target.value))
                        addPlayerToTeam(setPlayers, teams, setTeams, team.uuid);
                      if (!!players.find(v => v.uuid === e.target.value))
                        replacePlayerWithUuidTo(players, setPlayers, teams, setTeams, team.uuid, playerUuid, validate(e.target.value));
                      changePlayername(players, setPlayers, validate(e.target.value), playerUuid);
                    }}
                    onKeyDown={e => {
        ...
        <div className='flex-col team-div'>
          <div className='flex-row'>
            ...
            <button
              ...
              onClick={() => addTeam(setPlayers, teams, setTeams)}
            >{ '\u002B' }</button>
          </div>
        </div>
      </div>
