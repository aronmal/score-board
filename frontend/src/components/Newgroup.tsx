...
function New() {
  ...
  const checkForTeamnameDuplicates = (teamname: string) => (teams.filter(e => e.name === teamname).length >= 2)
  const checkForPlayernameDuplicates = (playerUuid: string) => (players[players.findIndex(e => e.uuid === playerUuid)].name !== '') ? (players.filter(e => e.name === players[players.findIndex(e => e.uuid === playerUuid)].name).length >= 2) : false
  const playernameOfUuid = (playerUuid: string) => players[players.findIndex(e => e.uuid === playerUuid)].name
  const isLastPlayerInTeam = (team: teamType, playerUuid: string) => (team.players[team.players.length - 1] === playerUuid)
            ...
            return (
              <div key={ playerUuid } className='player-in-team-div'>
                <input
                  ...
                  style={checkForPlayernameDuplicates(playerUuid) ? {color: 'red'} : {}}
                  type="text"
                  value={playernameOfUuid(playerUuid)}
                  placeholder='Spielername'
                  onChange={e => changePlayername(e, playerUuid)}
                  onKeyDown={e => {
                    if (e.code === 'Delete')
              ...
              <div key={ team.uuid } className='flex-col team-div team-div-real'>
                <div className='flex-row'>
                  ...
                  <input
                    ...
                    style={checkForTeamnameDuplicates(team.name) ? {color: 'red'} : {}}
                    type='text'
                    value={team.name}
                    placeholder='Teamname'
                    onChange={e => changeTeamname(e, team.uuid)}
                  />
                  <button
                  ...
                  {team.players.map(playerUuid => (
                    <div key={ playerUuid } className='player-in-team-div'>
                      <input
                        ...
                        placeholder='Spielername'
                        onChange={e => {
                          if ((isLastPlayerInTeam(team, playerUuid)) && !playernameOfUuid(playerUuid) && e.target.value)
                            addPlayerToTeam(team.uuid);
                          if (!!players.find(v => v.uuid === e.target.value))
                            replacePlayerWithUuidTo(e, team.uuid, playerUuid);
                          changePlayername(e, playerUuid);
                        }}
                        onKeyDown={e => {
                          if ((!isLastPlayerInTeam(team, playerUuid)) && (e.code === 'Delete'))
                            removePlayerFromTeam(team.uuid, playerUuid);
                        }}
                        onBlur={() => {
                          if ((!isLastPlayerInTeam(team, playerUuid)) && !playernameOfUuid(playerUuid))
                            removePlayerFromTeam(team.uuid, playerUuid);
                        }}
                      />
                      {(!isLastPlayerInTeam(team, playerUuid)) ?
                        <button
                          tabIndex={-1}
                          className='team-player-minus-button'
                          onClick={() => removePlayerFromTeam(team.uuid, playerUuid)}
                        ><span>{ '\u2A2F' }</span></button>
                      : <></>}
                    </div>
                  ))}
